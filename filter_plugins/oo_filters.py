...
def oo_lower_bound_cpu(value, limit):
    ''' Returns max(value, limit) respecting the unit conversions '''
    if not value:
        return value
    return max((norm_cpu(value), value), (norm_cpu(limit), limit))[1]


def oo_lower_bound_mem(value, limit):
    ''' Returns max(value, limit) respecting the unit conversions '''
    if not value:
        return value
...
class FilterModule(object):
    ...
    def filters(self):
        ...
        return {
            ...
            'oo_lower_bound_cpu': oo_lower_bound_cpu,
            'oo_lower_bound_mem': oo_lower_bound_mem
        }
