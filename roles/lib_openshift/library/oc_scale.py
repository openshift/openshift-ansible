...
# -*- -*- -*- Begin included fragment: lib/deployment.py -*- -*- -*-


# pylint: disable=too-many-public-methods
class Deployment(Yedit):
    ''' Class to model an openshift Deployment'''
    default_deployment_config = '''
...
'''
    ...
    def __init__(self, content=None):
        ...
        if not content:
            content = Deployment.default_deployment_config

        super(Deployment, self).__init__(content=content)
    ...
    def add_env_value(self, key, value):
        ...
        if env:
            ...
            result = self.put(Deployment.env_path, {'name': key, 'value': value})
            rval = result[0]
    ...
    def get_env_var(self, key):
        '''return a environment variables '''
        results = self.get(Deployment.env_path) or []
        if not results:
            return None
    ...
    def get_env_vars(self):
        '''return a environment variables '''
        return self.get(Deployment.env_path) or []
    ...
    def get_replicas(self):
        ''' return replicas setting '''
        return self.get(Deployment.replicas_path)
    ...
    def get_volumes(self, mounts=False):
        '''return volume mount information '''
        if mounts:
            return self.get(Deployment.volume_mounts_path) or []

        return self.get(Deployment.volumes_path) or []
    ...
    def add_volume_mount(self, volume_mount):
        ...
        if not exist_volume_mounts and volume_mount:
            self.put(Deployment.volume_mounts_path, [volume_mount])
        else:
            exist_volume_mounts.append(volume_mount)
    ...
    def add_volume(self, volume):
        ...
        if not exist_volumes:
            self.put(Deployment.volumes_path, [volume])
        else:
            exist_volumes.append(volume)

    def update_replicas(self, replicas):
        ''' update replicas value '''
        self.put(Deployment.replicas_path, replicas)
    ...
    def needs_update_replicas(self, replicas):
        ''' verify whether a replica update is needed '''
        current_reps = self.get(Deployment.replicas_path)
        return not current_reps == replicas

# -*- -*- -*- End included fragment: lib/deployment.py -*- -*- -*-
...
# -*- -*- -*- Begin included fragment: lib/replicaset.py -*- -*- -*-
...
# -*- -*- -*- End included fragment: lib/replicaset.py -*- -*- -*-
