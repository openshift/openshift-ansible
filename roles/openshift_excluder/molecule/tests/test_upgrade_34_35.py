...
testinfra_hosts = testinfra.utils.ansible_runner.AnsibleRunner(
    '.molecule/ansible_inventory').get_hosts('openshift_excluder_upgrade_34_35')

ansible_vars = testinfra.utils.ansible_runner.AnsibleRunner(
    '.molecule/ansible_inventory').get_variables('openshift_excluder_upgrade_34_35')
...
def test_docker_excluder(Package, Command):
    ...
    for key in keys:
        if key in ansible_vars and not ansible_vars[key]:
            ...
            # assuming the installation and upgrade are run with the same inventory file and group vars
            p = Package('atomic-openshift-docker-excluder')
            assert not p.is_installed
            return

    # check docker excluder 3.4 is installed
    p = Package('atomic-openshift-docker-excluder')
